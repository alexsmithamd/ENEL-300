...
int example_espnow_data_parse(uint8_t *data, uint16_t data_len, uint8_t *state, uint16_t *seq, uint32_t *magic)
{
    ...
    *magic = buf->magic;
    ...
    // Extract ultrasonic distance from payload
    ...
    if (payload_len >= sizeof(float)) {
        ...
        ESP_LOGD("US", "recieved ultrasonic distance: %f", g_ultrasonic_cm);
    } else {
        ...
        switch (evt.id) {
            ...
            case EXAMPLE_ESPNOW_RECV_CB:
            {
                ...
                if (ret == EXAMPLE_ESPNOW_DATA_BROADCAST) {
                    ...
                    float ultrasonic_copy;

                    taskENTER_CRITICAL(&ultrasonic_mux);
                    ultrasonic_copy = g_ultrasonic_cm;
                    taskEXIT_CRITICAL(&ultrasonic_mux);

                    ESP_LOGI(TAG, "Receive %dth unicast data from: "MACSTR", len: %d",
                            recv_seq, MAC2STR(recv_cb->mac_addr), recv_cb->data_len);

                    ESP_LOGI(TAG, "Received ultrasonic distance: %.2f cm", ultrasonic_copy);
